...
# TODO
	...
	Add shadowBias to light palette
...
# Ideas / future
	...
	Add light presets (saves all lights state) into slots or named sets
...
function EditScene(){
	...
	this.strokeSize = 1;
	this.checkContiguous = (localStorage_getItem('checkContiguous') !== 'false');
	this.paintAlpha = 1;
	this.strokeMode = 0;
	...
	this.autoStorePastePos = (localStorage_getItem('autoStorePastePos') === 'true');
	
	this.thumbnails = [];
	var renderTargetParameters = { minFilter: THREE.LinearFilter, magFilter: THREE.LinearFilter, format: THREE.RGBFormat, stencilBuffer: false };
	this.thumbnailCameraUserAngle = localStorage_getItem('thumbnailCameraUserAngle');
...
/* ------------------- ------------------- ------------------- ------------------- ------------------- Lights */
	...
	lightColorChanged:function(hsb, hex, rgb, div){ 
		localStorage_setItem(div.id, hex);
	...
	lightIntensityChanged:function(e, ui){
		...
		localStorage_setItem(targid, val);
	...
	updatePointLightPos:function(){
		...
		elev = localStorage_getItem('point-elevation');
		distance = localStorage_getItem('point-distance');
		direction = localStorage_getItem('point-direction');
	...
	updateDirectLightPos:function(){
		var elev, distance, direction, eu;
		elev = localStorage_getItem('direct-elevation');
		direction = localStorage_getItem('direct-direction');
	...
	updateSpotLightPos:function(){
		var elev, distance, direction, eu;
		elev = localStorage_getItem('spot-elevation');
		direction = localStorage_getItem('spot-direction');
		distance = localStorage_getItem('spot-distance');
...
/* ------------------- ------------------- ------------------- ------------------- ------------------- Painting */
	...
	checkContiguousChanged:function(e){
		...
		localStorage_setItem('checkContiguous', editScene.checkContiguous);
	},
...
/* ------------------- ------------------- ------------------- ------------------- ------------------- Selection */
	...
	copySelection:function(){
		...
		localStorage_setItem('clipboard', sf);
	},
	...
	pasteSelection:function(){
		...
		var pasteItem = localStorage_getItem('clipboard');
		if(!pasteItem) return;
		try {
	...
	autoStorePastePosChanged: function(){
		...
		localStorage_setItem('autoStorePastePos', this.autoStorePastePos);
	},
...
/* ------------------- ------------------- ------------------- ------------------- ------------------- Preview */
	...
	smoothNormalsChanged:function(){
		...
		localStorage_setItem('doc-smooth', editScene.doc.smoothNormals);
	...
	occlusionChanged:function(){
		...
		localStorage_setItem('doc-occlusion', editScene.doc.occlusion);		
	},
	...
	optimizeChanged:function(){
		...
		localStorage_setItem('doc-optimize', editScene.doc.optimize);
...
/* ------------------- ------------------- ------------------- ------------------- ------------------- Container & display functions */
	...
	toggleShowFloor:function(){
		localStorage_setItem('floorHidden',editScene.shadowPreviewPlane.visible);
...
/* ------------------- ------------------- ------------------- ------------------- ------------------- Anchors  */
	...
	anchorCopyValues:function(e){
		...
		localStorage_setItem('anchor-copy', JSON.stringify(obj));
		$('#anchor-paste').removeAttr('disabled');					
	},

	anchorPasteValues:function(e){
		var obj = localStorage_getItem('anchor-copy');
		if(!obj) return;
...
/* ------------------- ------------------- ------------------- ------------------- ------------------- Frames functions */
	...
	toggleFrameRange: function(){
		...
		localStorage_setItem('timeline-show-range', !coll);
		if(coll) { 
			$('#frame-range').hide();
	...
	frameRangeCopy:function(e){
		...
		localStorage_setItem('framePaste', JSON.stringify({f:copiedFrames, e:extras}));
	},

	frameRangePaste:function(e, startFrame, frames){
		if(e){
			frames = localStorage_getItem('framePaste');
...
/* ------------------- ------------------- ------------------- ------------------- ------------------- Document functions */
	...
	holdDoc:function(){	
		function doHold(){
			...
			localStorage_setItem('holdDoc', data);
		}
		
		if(localStorage_getItem('holdDoc')){
	...
	newDoc:function(w,h,d){
		...
		if(arguments.length == 3){
			w = Math.abs(w || 8); localStorage_setItem('new-width', w);
			h = Math.abs(h || 8); localStorage_setItem('new-height', h);
			d = Math.abs(d || 8); localStorage_setItem('new-depth', d);
			...
			this.doc = { 
				...
				floor: false,
				optimize: (localStorage_getItem('doc-optimize') != null ? (localStorage_getItem('doc-optimize') === 'true') : true),
				smoothNormals: (localStorage_getItem('doc-smooth') != null ? parseFloat(localStorage_getItem('doc-smooth')) : 1.0),
				occlusion: (localStorage_getItem('doc-occlusion') != null ? parseFloat(localStorage_getItem('doc-occlusion')) : 1.0),
				pointSize: 1.0,
				frames:[],
				anims:[]
			};
			...
			if(!$('#new-doc').length){
				...
				<label for="new-width" class="w2 right-align">Width</label><input id="new-width" name="new-width" size="1"/><br/>\
				<label for="new-height" class="w2 right-align">Height</label><input id="new-height" name="new-height" size="1"/><br/>\
				<label for="new-depth" class="w2 right-align">Depth</label><input id="new-depth" name="new-depth" size="1"/><br/>\
			...
			$('#new-width').val(localStorage_getItem('new-width') || 8);
		    $('#new-height').val(localStorage_getItem('new-height') || 8);
		    $('#new-depth').val(localStorage_getItem('new-depth') || 8);
	    }
	},
	...
	newDocFromData:function(dataObject){
		...
		$('#preview-smooth-normals').spinner('value', this.doc.smoothNormals * 100);
		$('#preview-occlusion').spinner('value', this.doc.occlusion * 100);
		$('#preview-point-size').spinner('value', this.doc.pointSize);
		$('#preview-optimize').prop('checked', this.doc.optimize);
		$('#preview-floor').prop('checked', this.doc.floor);
	},
...
/* ------------------- ------------------- ------------------- ------------------- ------------------- UI functions */
	...
	// menu bar
		...
		<ul class="editor absolute-pos submenu shortcuts" id="file-submenu">\
			<li id="file-new">New <em>Ctrl + N</em></li>\
			<li id="file-putback" style="display:none;">Save Changes / Update Scene</li>\
			<hr/>\
		...
		$('#edit-resize').click(editScene.resizeDoc.bind(editScene));
		$('#file-load').click(editScene.loadDoc);
		$('#file-save').click(editScene.saveDoc);
		$('#file-putback').click(editScene.putBack);
	...
	// frames toolbar
		...
		'<div id="editor-toolbar" class="ui-widget-header ui-corner-all editor floating-panel"><h1>Frame</h2>\
		  ...
		  <input id="current-frame" size="1" class="center"/><label id="total-frames" for="current-frame" class="w1"> of '+editScene.doc.frames.length+'</label>\
		...
		var coll = localStorage_getItem('editor-toolbar-collapsed'); if(coll == 'true') $('#editor-toolbar').addClass('collapsed');
		coll = localStorage_getItem('timeline-show-range');
		if(coll === 'true') { 
		...
		var dx = localStorage_getItem('editor-toolbar-x');
		var dy = localStorage_getItem('editor-toolbar-y');
	...
	// stroke
		...
		coll = localStorage_getItem('editor-stroke-collapsed'); if(coll == 'true') $('#editor-stroke').addClass('collapsed');
		dw = $('#editor-stroke').width(); dh = $('#editor-stroke').height();
		dx = localStorage_getItem('editor-stroke-x'); dy = localStorage_getItem('editor-stroke-y');
		...
		<hr class="clear"/>\
		<label for="mask-x" class="w3">X&nbsp;<em>min&nbsp;</em></label><input id="mask-x" size="1" class="center" value="0"/>\
		<label for="mask-x2"><em>&nbsp;max&nbsp;</em></label><input id="mask-x2" size="1" class="center"/><br\>\
		<label for="mask-y" class="w3">Y&nbsp;<em>min&nbsp;</em></label><input id="mask-y" size="1" class="center" value="0"/>\
		<label for="mask-y2"><em>&nbsp;max&nbsp;</em></label><input id="mask-y2" size="1" class="center"/><br\>\
		<label for="mask-z" class="w3">Z&nbsp;<em>min&nbsp;</em></label><input id="mask-z" size="1" class="center" value="0"/>\
		<label for="mask-z2"><em>&nbsp;max&nbsp;</em></label><input id="mask-z2" size="1" class="center"/>\
	...
	// masking
		...
		coll = localStorage_getItem('editor-mask-collapsed'); if(coll == 'true') $('#editor-mask').addClass('collapsed');
		dw = $('#editor-mask').width(); dh = $('#editor-mask').height();
		dx = localStorage_getItem('editor-mask-x'); dy = localStorage_getItem('editor-mask-y');
		dx = Math.min((dx === null) ? (20) : dx, window.innerWidth - dw);
	...
	// paste 
		...
		<h1>Paste</h1>\
		<hr/>\
		<label for="paste-x" class="w0 pad5 right-align">X</label><input id="paste-x" size="1" class="center" value="0"/>\
		<label for="paste-rx" class="w1 pad5 right-align">Rot X</label><input id="paste-rx" size="1" class="center" value="0"/>\
		<label for="paste-sx" class="w2 pad5 right-align">Scale X</label><input id="paste-sx" size="1" class="center" value="0"/><br/>\
		<label for="paste-y" class="w0 pad5 right-align">Y</label><input id="paste-y" size="1" class="center" value="0"/>\
		<label for="paste-ry" class="w1 pad5 right-align">Rot Y</label><input id="paste-ry" size="1" class="center" value="0"/>\
		<label for="paste-sy" class="w2 pad5 right-align">Scale Y</label><input id="paste-sy" size="1" class="center" value="0"/><br/>\
		<label for="paste-z" class="w0 pad5 right-align">Z</label><input id="paste-z" size="1" class="center" value="0"/>\
		<label for="paste-rz" class="w1 pad5 right-align">Rot Z</label><input id="paste-rz" size="1" class="center" value="0"/>\
		<label for="paste-sz" class="w2 pad5 right-align">Scale Z</label><input id="paste-sz" size="1" class="center" value="0"/><br/>\
		<hr/>\
		...
		coll = localStorage_getItem('editor-paste-collapsed'); if(coll == 'true') $('#editor-paste').addClass('collapsed');
		dw = $('#editor-paste').width(); dh = $('#editor-paste').height();
		dx = localStorage_getItem('editor-paste-x'); dy = localStorage_getItem('editor-paste-y');
		dx = Math.min((dx === null) ? (window.innerWidth - dw)*0.5 : dx, window.innerWidth - dw);
		dy = Math.min((dy === null) ? (window.innerHeight - dh - 20) : dy, window.innerHeight - dh);
		editScene.pasteUI
			.offset({left:dx, top: dy })
	...
	// preview
		...
		<input id="preview-smooth-normals" size="1" class="center" value="'+Math.floor(editScene.doc.smoothNormals*100)+'"/><br/>\
		<label for="preview-occlusion" class="w4 right-align">Occlusion&nbsp;&nbsp;</label>\
		<input id="preview-occlusion" size="1" class="center" value="'+Math.floor(editScene.doc.occlusion*100)+'"/><br/>\
		<label for="preview-point-size" class="w4 right-align">Point Size&nbsp;&nbsp;</label>\
		<input id="preview-point-size" size="1" class="center" value="'+Math.floor(editScene.doc.pointSize)+'"/><br/>\
		...
		coll = localStorage_getItem('editor-preview-collapsed'); if(coll == 'true') $('#editor-preview').addClass('collapsed');
		dw = $('#editor-preview').width(); dh = $('#editor-preview').height();
		dx = localStorage_getItem('editor-preview-x'); dy = localStorage_getItem('editor-preview-y');
	...
	// light control
		...
		<label class="w3 right-align" for="spot-shadow">Shadow</label>&nbsp;<input type="checkbox" id="spot-shadow" '+((localStorage_getItem('spot-shadow') !== 'false') ? 'checked="checked"':'')+'/><br/>\
		...
		<label class="w3 right-align" for="direct-shadow">Shadow</label>&nbsp;<input type="checkbox" id="direct-shadow" '+((localStorage_getItem('direct-shadow') !== 'false') ? 'checked="checked"':'')+'/><br/>\
		...
		coll = localStorage_getItem('editor-lights-collapsed'); if(coll == 'true') $('#editor-lights').addClass('collapsed'); else if(coll == 'false') $('#editor-lights').removeClass('collapsed');
		dw = $('#editor-lights').width(); dh = $('#editor-lights').height();
		dx = localStorage_getItem('editor-lights-x'); dy = localStorage_getItem('editor-lights-y');
	...
	// animations
		...
		<label for="anim-fps" class="w1 right-align">FPS&nbsp;</label><input id="anim-fps" size="1" class="center" value=""/><br/>\
		<hr/>\
		<label for="anim-start" class="w2 right-align">Start&nbsp;</label><input id="anim-start" size="1" class="center" value=""/>\
		<label for="anim-len" class="w2 right-align">Length&nbsp;</label><input id="anim-len" size="1" class="center" value=""/><br/>\
		...
		coll = localStorage_getItem('editor-anims-collapsed'); if(coll == 'true') $('#editor-anims').addClass('collapsed'); else if(coll == 'false') $('#editor-anims').removeClass('collapsed');
		dw = $('#editor-anims').width(); dh = $('#editor-anims').height();
		dx = localStorage_getItem('editor-anims-x'); dy = localStorage_getItem('editor-anims-y');
		dx = Math.min((dx === null) ? (window.innerWidth - dw - 20) : dx, window.innerWidth - dw);
	...
	// anchors
		...
		<hr/>\
		<div class="column">\
		<label for="anchor-x" class="w1 right-align">X&nbsp;</label><input id="anchor-x" size="1" class="center" value=""/><br/>\
		<label for="anchor-y" class="w1 right-align">Y&nbsp;</label><input id="anchor-y" size="1" class="center" value=""/><br/>\
		<label for="anchor-z" class="w1 right-align">Z&nbsp;</label><input id="anchor-z" size="1" class="center" value=""/></div>\
		<div class="column">\
		<label for="anchor-rx" class="w2 right-align">Rot X&nbsp;</label><input id="anchor-rx" size="1" class="center" value=""/><br/>\
		<label for="anchor-ry" class="w2 right-align">Rot Y&nbsp;</label><input id="anchor-ry" size="1" class="center" value=""/><br/>\
		<label for="anchor-rz" class="w2 right-align">Rot Z&nbsp;</label><input id="anchor-rz" size="1" class="center" value=""/></div>\
		<div class="column">\
		<label for="anchor-sx" class="w2 right-align">Scale X&nbsp;</label><input id="anchor-sx" size="1" class="center" value=""/><br/>\
		<label for="anchor-sy" class="w2 right-align">Scale Y&nbsp;</label><input id="anchor-sy" size="1" class="center" value=""/><br/>\
		<label for="anchor-sz" class="w2 right-align">Scale Z&nbsp;</label><input id="anchor-sz" size="1" class="center" value=""/></div>\
		<div class="center scale75 clear"><a id="anchor-toall">&larr; copy anchor values to all frames &rarr;</a><span class="separator-left"/>\
		<a id="anchor-copy">copy</a>&nbsp;&nbsp;&nbsp;&nbsp;<a id="anchor-paste" '+(localStorage_getItem('anchor-paste') ? '' : 'disabled="disabled"')+'>paste</a></div>\
		<hr/>\
		...
		coll = localStorage_getItem('editor-anchors-collapsed'); if(coll == 'true') $('#editor-anchors').addClass('collapsed'); else if(coll == 'false') $('#editor-anchors').removeClass('collapsed');
		dw = $('#editor-anchors').width(); dh = $('#editor-anchors').height();
		dx = localStorage_getItem('editor-anchors-x'); dy = localStorage_getItem('editor-anchors-y');
		dx = Math.min((dx === null) ? (window.innerWidth - dw - 20) : dx, window.innerWidth - dw);
	...
	// add collapse buttons
		...
		$('.toggleCollapse').click(function(){
			...
			localStorage_setItem(parent.get(0).id+'-collapsed', collapsed);
		});
...
/* ------------------- ------------------- ------------------- ------------------- ------------------- Keyboard functions */
	...
	enableKeyboardShortcuts:function(){
		...
		key('⌘+w', function(){ window.close(); return false; });
		...
		key('ctrl+s,⌘+s', function(){ if(window.loadAsset) editScene.putBack(); else editScene.holdDoc(); return false; });
	...
	/* shortcuts */
	keyUp:function(e){
		if(editScene.focusedTextField || editScene.stroking || editScene.movingMask 
			|| editScene.movingPaste || (e.target && (e.target.tagName == 'INPUT' || e.target.tagName == 'TEXTAREA'))) return;
...
/* ------------------- ------------------- ------------------- ------------------- ------------------- Framework */
	...
	init:function(){
		// defaults
		var bgc = localStorage_getItem('editor-bg-color');
		...
		// point
		var pointColor = localStorage_getItem('point-color');
		intensity = localStorage_getItem('point-intensity');
		var falloff = localStorage_getItem('point-falloff');
	    ...
	    // spot
		pointColor = localStorage_getItem('spot-color');
		intensity = localStorage_getItem('spot-intensity');
		falloff = localStorage_getItem('spot-falloff');
		var angle = localStorage_getItem('spot-angle');
		var exp = localStorage_getItem('spot-exp');
		...
		this.spot.castShadow = true;//(localStorage_getItem('spot-shadow') !== 'false');
		...
		this.anchors.visible = (localStorage_getItem('show-anchors') !== 'false');
...
function documentReady(){
	...
	localStorage_init(function(){
		window.editScene = new EditScene();	
		editScene.init();
